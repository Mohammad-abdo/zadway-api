...
import * as service from "./product-variants.service.js";
import { successResponse, paginatedResponse, errorResponse } from "../../core/utils/serverResponse.js";
import { t } from "../../core/i18n/index.js";

/** @param {unknown} err */
function duplicateVariantMessage(err, locale) {
  if (!(err instanceof Prisma.PrismaClientKnownRequestError) || err.code !== "P2002") return null;
  const target = err.meta?.target;
  const fields = Array.isArray(target) ? target : [];
  if (fields.includes("sku")) return t("productVariants.duplicate_sku", locale);
  if (fields.includes("productId")) return t("productVariants.duplicate_product", locale);
  return t("db.duplicate", locale);
}
...
export async function create(req, res) {
  try {
    ...
    const dup = duplicateVariantMessage(e, req.locale);
    if (dup) return errorResponse(res, dup, 409);
    return errorResponse(res, e.message, 500);
  }
}
